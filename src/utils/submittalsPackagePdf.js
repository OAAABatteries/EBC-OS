 ...
 */
export function generateSubmittalsPackagePdf(project, submittals = [], contacts = [], company = {}) {
  const doc = new jsPDF({ unit: "mm", format: "letter" });

  // ── Company defaults ──
  const companyName  = company.name    || "Eagles Brothers Constructors";
  const companyAddr  = company.address || "Houston, TX";
  const companyPhone = company.phone   || "713-589-4858";
  ...
  doc.setTextColor(...C.amber[0] > 200 ? C.amber : C.amber2);
...
function _drawTableHeader(doc, y) {
  ...
  const cols = _rowCols();
  const ML_ = ML;
  doc.text("#",          ML_ + cols.num,       y + 5);
  doc.text("Spec",       ML_ + cols.spec,      y + 5);
  doc.text("Description",ML_ + cols.desc,      y + 5);
  doc.text("Type",       ML_ + cols.type,      y + 5);
  doc.text("Status",     ML_ + cols.status,    y + 5);
  doc.text("Submitted",  ML_ + cols.submitted, y + 5);
  doc.text("Returned",   ML_ + cols.returned,  y + 5);
}
