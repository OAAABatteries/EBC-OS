...
const CARD_STYLES = `
  ...
  .bc-divider { width:40px;height:2px;background:#f59e0b;margin: var(--space-3) 0;border-radius:2px; }
  .bc-info { margin-top:auto;display:flex;flex-direction:column;gap: var(--space-1); }
  .bc-info-row { font-size:10px;color:#94a3b8;display:flex;align-items:center;gap: var(--space-2); }
...
export default function BusinessCardGenerator({ employees, app }) {
  ...
  const handlePrint = () => {
    ...
    printWin.document.write(`<html><head><style>${CARD_STYLES} body{margin: var(--space-5);background:#fff;}</style></head><body>`);
